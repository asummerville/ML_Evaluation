---
title: "Machine Learning Evaluation Lab"
...
## In-class Example (bank data)

/basic info about the goal of the model and what metrics we think should matter (hypothesis). ex. we dont want false positives bc they could lose money if they target/accept the wrong customers.
...
prob_knn <- tibble(bank_5NN, attributes(bank_5NN)$prob)
prob_knn
prob_knn$prob <- if_else(prob_knn$bank_5NN==0,1-prob_knn$`attributes(bank_5NN)$prob`, prob_knn$`attributes(bank_5NN)$prob`)
view(prob_knn)

```
/basic analysis of the stats and what they mean before we go into deep analysis below
...
f1 #score of 0.31 ... not very good
...
## kNN lab analysis (commercial data)

/basic info about the goal of the model and what metrics we think should matter (hypothesis).
...
prob_knn2 <- tibble(CNN_KNN_optimal, attributes(CNN_KNN_optimal)$prob)
prob_knn2 <- as.data.frame(prob_knn2)
str(prob_knn2)
view(prob_knn2)
prob_knn2$prob <- if_else(prob_knn$CNN_KNN_optimal==-1,1-prob_knn$`attributes(CNN_KNN_optimal)$prob`, prob_knn$`attributes(CNN_KNN_optimal)$prob`)
...
#accuracy, TPR, FPR, F1, Kappa, ROC/AUC
